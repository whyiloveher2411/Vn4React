...
(function ($, Modernizr, image_editor)
{
	...
	var FileManager = {
		...
		contextActions: {
			...
			tiny_image: function($trigger){


				if( $trigger.find('.img-precontainer-mini').hasClass('directory') ){
					console.log($trigger);
					var m = $trigger.data('path'), type = 'directory';

				}else{
					var m = jQuery('#sub_folder').val() + jQuery('#fldr_value').val() + $trigger.find('a.link').attr('data-file');

					if( $trigger.find('.img-precontainer-mini .filetype').hasClass('jpg') ){
						var type = 'jpg';
					}else{
						var type = 'png';
					}
				...
				$.ajax({
					type: "POST",
					url: jQuery('#url_tiny_image').val(),
					dataType:'Json',
					data: {
						_token: jQuery('#laravel_token').val(),
						path: m,
						type: type,
						dir: jQuery('#laravel_dir').val(),
					},
				}).done(function (data)
				{
					hide_animation();
			...
			$.contextMenu({
				...
				build: function ($trigger)
				{
					...
					if ( $trigger.find('.img-precontainer-mini .filetype').hasClass('png') ||
						$trigger.find('.img-precontainer-mini .filetype').hasClass('jpg')  || $trigger.find('.img-precontainer-mini').hasClass('directory') )
					{
						options.items.tiny_image = {
							name: 'Compress Image',
							icon: "tiny_image",
							disabled: false
						};
	...
	jQuery(document).ready(function ()
	{
		...
		jQuery('.selector').on('click',function(e){
			...
			if(jQuery('.selection:checkbox:checked:visible').length>0){
				jQuery("#multiple-selection .tip.btn.multiple-delete-btn").show(300);
			}else{
				jQuery("#multiple-selection .tip.btn.multiple-delete-btn").hide(300);
			}
	...
	apply_any = function(url)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					parent.document.getElementsByClassName('tox-textfield')[0].value = url;
					parent.document.getElementsByClassName('tox-textfield')[3].value = null;
					parent.document.getElementsByClassName('tox-textfield')[4].value = null;
					// $(parent).find('.tox-dialog__body-content input').val(url);
					// console.log(parent.tinymce.activeEditor.getParam('url',url));
					// parent.tinymce.activeEditor.getParam('url',url);
					// parent.tinymce.activeEditor.windowManager.getParams().setUrl(url);
					// parent.tinymce.activeEditor.insertContent('dfs');
					parent.tinymce.activeEditor.windowManager.close();
				}
			}
		}
