...
$reactID = Request::get('id');

if( $access_token ){

	$decoded = \Firebase\JWT\JWT::decode($access_token, env('APP_KEY'), array('HS256'),true);

	if( $decoded->expires_time < time() ){
		echo '<h1 style="text-align:center;margin-top: 100px;"><script>window.parent.postMessage({type:"REQUIRED_LOGIN", id: "'.$reactID.'",value: "...."},"*");</script>Please reopen to continue</h1>';
		exit;
	}
	...
	setcookie('access_filemanage',$access_token, $decoded->expires_time*1 , "/");
	
}else{

	$access_token = isset($_COOKIE['access_filemanage']) ? $_COOKIE['access_filemanage'] : false;

}

if( !$access_token ){
	echo '<h1 style="text-align:center;margin-top: 100px;"><a href="javascript:window.location.reload()">Please reopen to continue</a></h1>';
	// header('Location: /');
	exit;
}
...
if( File::exists($file = cms_path().'/../filemanager2/filemanager/lang/'.$transFile.'.php') ){
...
$config['base_url'] = str_replace('/public/filemanager2/filemanager/dialog.php', '', env('APP_URL'));
...
<script src="js/jquery.fileupload-ui.js"></script>
	...
	<input type="hidden" id="react_id" value="<?php echo $reactID; ?>" />
	<input type="hidden" id="url_tiny_image" value="<?php echo str_replace('public/filemanager2/filemanager/dialog.php/', '', route('admin.page',['page'=>'tool-genaral','action'=>'tiny-image']));?>" />
					    ...
					    <tr class="template-download fade">
					       <td>
				                ...
				                {% } else { %}
				                	...
				                	<a href="javaScript:void(0)" class="link new-file" title="{%=file.name%}" data-file="{%=file.name%}" data-function="apply" {%=file.thumbnailUrl?'data-gallery':''%}><img src="<?php echo $config['base_url'].'/filemanager2/filemanager/img/ico/' ?>{%=file.extension%}.jpg" style="width: 47px;height: auto;"></a>
...
?>
			<li class="ff-item-type-<?php echo $class_ext;?> file <?php if(!$config['multiple_selection']){ ?>no-selector<?php } ?>" data-full-path="<?php echo $cur_dir.$file; ?>"   data-name="<?php echo $file;?>" <?php if(($filter!='' && stripos($file,$filter)===false)) echo ' style="display:none;"';?>><?php
			$file_prevent_rename = false;
			$file_prevent_delete = false;
			if (isset($filePermissions[$file])) {
...
<script>
	...
	<script>
		...
		const eventListenerMessage = (event) => {
			...
			switch (data.type) {
				case 'DATA_VALUE':
					window.values = data.images;
					console.log(data);

					if( values ){

						$('.choise-multi').removeClass('choise-multi');
						$('.loading-img-warpper').removeClass('loading-img-warpper');
						$('.loading-img').remove();

						let listImage = {};

						Object.keys( values ).forEach( key => {
							let name = unescape(values[key].link);
							let liTag = $('.ff-item-type-2[data-full-path="'+name+'"]');

							if( listImage[name] ){
								listImage[name] = listImage[name] + 1; 
							}else{
								listImage[name] = 1; 
							}

							liTag.find('.soluong').remove();

							if( listImage[name] > 1 ){
								liTag.append( '<span class="soluong">'+listImage[name]+'</span>' );
							}

							liTag.addClass('choise-multi');
							liTag.removeClass('loading-img-warpper');
							liTag.find('.loading-img').remove();

						});
						console.log(listImage);
						// if( data.result ){
						// 	values[data.image.link] = data.image;
						// 	objectImage[data.imageLink].toggleClass('choise-multi');
						// }
						// objectImage[data.imageLink].removeClass('loading-img-warpper');
						// objectImage[data.imageLink].find('.loading-img').remove();
					}

					break;
				case 'VALIDATE_IMAGE_RESULT':
		...
		$('ul#main-item-container.grid.cs-style-2.list-view0 .ff-item-type-2').click(function(event){
			...
			$this.addClass('loading-img-warpper');
			$this.append('<img class="loading-img" src="img/loading2.gif">');
			// alert($(this).find('.link').data("file"));
			apply_img3($(this).find('.link').data("file"),jQuery("#field_id").val(), function(url){
				// alert(url);

				window.objectImage[url] = $this;
				...
				// after_validate_image_multi(url, $this);
			});
		});
		...
		function after_validate_image_multi(url, $this){
