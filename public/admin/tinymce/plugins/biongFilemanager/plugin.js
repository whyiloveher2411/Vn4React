 ...
 *
 * Contributing: https://github.com/trippo/biongFilemanager
 */

tinymce.PluginManager.add('biongFilemanager', function(editor) {

	function biongFilemanager_onMessage(event){
		if(editor.settings.external_filemanager_path.toLowerCase().indexOf(event.origin.toLowerCase()) === 0){
			if(event.data.sender === 'biongFilemanager'){
				tinymce.activeEditor.insertContent(event.data.html);
				tinymce.activeEditor.windowManager.close();

				// Remove event listener for a message from biongFilemanager
				if(window.removeEventListener){
					window.removeEventListener('message', biongFilemanager_onMessage, false);
				} else {
					window.detachEvent('onmessage', biongFilemanager_onMessage);
				}
			}
		}
	}
	...
	function openmanager() {
		...
		if (typeof editor.settings.filemanager_crossdomain !== "undefined" && editor.settings.filemanager_crossdomain) {
			...
			// Add handler for a message from biongFilemanager
			if(window.addEventListener){
				window.addEventListener('message', biongFilemanager_onMessage, false);
			} else {
				window.attachEvent('onmessage', biongFilemanager_onMessage);
			}
		}
	...
	if (tinymce.majorVersion < 5) {
		editor.addButton('biongFilemanager', {
			icon: 'browse',
			tooltip: 'Insert file',
			shortcut: 'Ctrl+E',
			onClick: openmanager
		...
		editor.addMenuItem('biongFilemanager', {
			icon: 'browse',
			text: 'Insert file',
			shortcut: 'Ctrl+E',
			onClick: openmanager,
		...
		editor.ui.registry.addButton('biongFilemanager', {
			icon: 'browse',
			tooltip: 'Insert file',
			shortcut: 'Ctrl+E',
		...
		editor.ui.registry.addMenuItem('biongFilemanager', {
			icon: 'browse',
			text: 'Insert file',
			shortcut: 'Ctrl+E',
			onAction: openmanager,
