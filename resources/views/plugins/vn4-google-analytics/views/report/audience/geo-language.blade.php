...
$dataGoogle = multiple_threads_request(
	[
		...
		'data_language_total'=>get_url_google_analytics(
		'https://www.googleapis.com/analytics/v3/data/ga',
		$webpropertie_id,
...
$data_language_total = [];
$totalsForAllResults = $dataGoogle['data_language_total']['totalsForAllResults'];
$totalsForAllResults2 = $dataGoogle['data_total']['totalsForAllResults'];
foreach ($dataGoogle['data_language_total']['rows'] as $k => $v) {
	foreach ($dataGoogle['data_language_total']['columnHeaders'] as $k2 => $v2) {
		$data_language_total[$v2['name']][$k] = $v[$k2];
	}
}
...
<div class="table_chart">
	<table style="width: 100%;" >
		...
		<tbody>
			...
			@foreach($data_language_total['ga:language'] as $k => $v)
			<tr>
				<td>{!!$v!!}</td>
				<td>{!!number_format($data_language_total['ga:users'][$k])!!} <span>@if( $totalsForAllResults['ga:users'] != 0) ({!!number_format($data_language_total['ga:users'][$k] * 100 / $totalsForAllResults['ga:users'],2)!!}%) @endif</span></td>
				<td>{!!number_format($data_language_total['ga:newUsers'][$k])!!} <span>@if( $totalsForAllResults['ga:newUsers'] != 0) ({!!number_format($data_language_total['ga:newUsers'][$k] * 100 / $totalsForAllResults['ga:newUsers'],2)!!}%) @endif</span></td>
				<td>{!!number_format($data_language_total['ga:sessions'][$k])!!} <span>@if( $totalsForAllResults['ga:sessions'] != 0) ({!!number_format($data_language_total['ga:sessions'][$k] * 100 / $totalsForAllResults['ga:sessions'],2)!!}%) @endif</span></td>
				<td>{!!number_format($data_language_total['ga:bounceRate'][$k],2)!!}%</td>
				<td>{!!number_format($data_language_total['ga:pageviewsPerSession'][$k],2)!!}</td>
				<?php 
					$time = number_format($data_language_total['ga:avgSessionDuration'][$k]);
					$hours = floor($time / 3600);
					$mins = floor($time / 60 % 60);
					$secs = floor($time % 60);
					$timeFormat = sprintf('%02d:%02d:%02d', $hours, $mins, $secs);
				 ?>
				<td>{!!$timeFormat!!}</td>
				<td>{!!number_format($data_language_total['ga:goal1ConversionRate'][$k],2)!!}%</td>
				<td>{!!number_format($data_language_total['ga:goal1Completions'][$k])!!} <span>@if( $totalsForAllResults['ga:goal1Completions'] != 0) ({!!number_format($data_language_total['ga:goal1Completions'][$k] * 100 / $totalsForAllResults['ga:goal1Completions'],2)!!}%) @endif</span></td>
				<td>${!!number_format($data_language_total['ga:goal1Value'][$k],2)!!} @if( $totalsForAllResults['ga:goal1Value'] != 0) <span>({!!number_format($data_language_total['ga:goal1Value'][$k] * 100 / $totalsForAllResults['ga:goal1Value'],2)!!}%)</span> @else <span>(0.00%)</span> @endif</td>
			</tr>
			@endforeach
		</tbody>
	 </table>
