...
<div id="chart_current_temp" class="chart_area" style="width:100%;visibility: hidden;position: absolute;"></div>
 ...
 <script type="text/javascript">
  ...
  function drawChart() {
  	...
  	$add_aera_chart = function($title, $data,$index, $id_frame_img, $id_chart,$mode, $param = null, $start_date, $end_date ) use ($__env){ 
		...
		options_chuan['{!!$id_frame_img!!}'] = options;
		 ...
		 var data = new google.visualization.DataTable();
	      ...
	      @elseif( $mode === 'ga:date' )
		      @if( isset($param['customData']) )
			      @foreach($data['rows'] as $k => $v)
					[{v:new Date({!!date("Y, m, d", strtotime($v[0]))!!}),f:'{!!date("l, F d, Y", strtotime($v[0]))!!}'},  {!!$param['customData']($v[$index])!!}],
			      @endforeach
		      @else
			      @foreach($data['rows'] as $k => $v)
			      [{v:new Date({!!date("Y, m, d", strtotime($v[0]))!!}),f:'{!!date("l, F d, Y", strtotime($v[0]))!!}'},  {!!$v[$index]!!}],
			      @endforeach
		      @endif
	      @elseif( $mode === 'ga:week' )
      		  @if( isset($param['customData']) )
	      ...
	      @endif
			...
			// hAxis: {title: '',format:'0',  gridlines: {count:2,color: 'transparent'}, ticks: [ 
   //              { v: new Date(2019,11,23,0,0,0), f: 'Now 23'}, 
   //          ]},
            
			// console.log(data);
		window.data_chuan['{!!$id_frame_img!!}'] = data;
