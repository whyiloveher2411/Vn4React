...
$dataGoogle = multiple_threads_request(
	...
	[
		...
		'data_age_total'=>get_url_google_analytics(
...
$totalsForAllResults = $dataGoogle['data_age_total']['totalsForAllResults'];

$totalsForAllResults2 = $dataGoogle['data_total']['totalsForAllResults'];



foreach ($dataGoogle['data_age_total']['rows'] as $k => $v) {

	foreach ($dataGoogle['data_age_total']['columnHeaders'] as $k2 => $v2) {

		$data_age_total[$v2['name']][$k] = $v[$k2];
...
<div class="table_chart">
	...
	<table style="width: 100%;" >
		...
		<tbody>
			...
			@foreach($data_age_total['ga:userAgeBracket'] as $k => $v)

			<tr>

				<td>{!!$v!!}</td>

				<td>{!!number_format($data_age_total['ga:users'][$k])!!} <span>@if( $totalsForAllResults['ga:users'] != 0) ({!!number_format($data_age_total['ga:users'][$k] * 100 / $totalsForAllResults['ga:users'],2)!!}%) @endif</span></td>

				<td>{!!number_format($data_age_total['ga:newUsers'][$k])!!} <span>@if( $totalsForAllResults['ga:newUsers'] != 0) ({!!number_format($data_age_total['ga:newUsers'][$k] * 100 / $totalsForAllResults['ga:newUsers'],2)!!}%) @endif</span></td>

				<td>{!!number_format($data_age_total['ga:sessions'][$k])!!} <span>@if( $totalsForAllResults['ga:sessions'] != 0) ({!!number_format($data_age_total['ga:sessions'][$k] * 100 / $totalsForAllResults['ga:sessions'],2)!!}%) @endif</span></td>

				<td>{!!number_format($data_age_total['ga:bounceRate'][$k],2)!!}%</td>

				<td>{!!number_format($data_age_total['ga:pageviewsPerSession'][$k],2)!!}</td>



				<?php 

					$time = number_format($data_age_total['ga:avgSessionDuration'][$k]);
				...
				<td>{!!number_format($data_age_total['ga:goal1ConversionRate'][$k],2)!!}%</td>

				<td>{!!number_format($data_age_total['ga:goal1Completions'][$k])!!} <span>@if( $totalsForAllResults['ga:goal1Completions'] != 0) ({!!number_format($data_age_total['ga:goal1Completions'][$k] * 100 / $totalsForAllResults['ga:goal1Completions'],2)!!}%) @endif</span></td>

				<td>${!!number_format($data_age_total['ga:goal1Value'][$k],2)!!} @if( $totalsForAllResults['ga:goal1Value'] != 0) <span>({!!number_format($data_age_total['ga:goal1Value'][$k] * 100 / $totalsForAllResults['ga:goal1Value'],2)!!}%)</span> @else <span>(0.00%)</span> @endif</td>
...
<script type="text/javascript">
	...
	google.charts.load('current', {'packages':['corechart']});
  	...
  	function drawChart(){
	      ...
	      [{type: 'datetime', label: 'Date'},'User','18-24','25-34','35-44','45-54','55-64','65+'],

	      @foreach($data_new as $k => $v)

	      [{v:new Date('{!!date("M d, Y", strtotime($k))!!}'),f:'{!!date("l, F d, Y", strtotime($k))!!}'}, {!!$v['18-24']+$v['25-34']+$v['35-44']+$v['45-54']+$v['55-64']+($v['65+']??0)!!} , {!!$v['18-24']!!}, {!!$v['25-34']!!}, {!!$v['35-44']!!}, {!!$v['45-54']!!}, {!!$v['55-64']!!}, {!!$v['65+']??0!!}],
	    ...
	    var options = {
	      ...
	      colors:['#058DC7','#50B432','#f09745','#AF49C5','#EDEF00','#8080FF','#A0A424'],
