...
<div style="width: 32.999%;text-align: center;display: inline-block;float: left;box-sizing: border-box;padding: 0 5px 0 0">
	...
	<div style="font-size: 75px;" id="total_">{!!$dk_total?$total:0!!}</div>
	...
	<div style="padding: 25px 15px;">
		<div class="" style="text-align: left;margin-bottom:3px;" >
          ...
          if( $filter ){
            $value = [$filter => [$total,'rgb(5, 141, 199, 1)']];
          }else{
            $value = $arg['count1']['value'];
            arsort($value);
            $opacity = 1;
            foreach ($value as $k => $v) {
              $value[$k] = [$v,'rgb(5, 141, 199, '.$opacity.')'];
              $opacity -= 0.2;
            }
          }
			...
			@foreach($value as $k => $v )
      <a href="?filter={!!$k!!}">
			<div class="device_item2" style="color: #005c9c;display: inline-block;text-transform: uppercase;font-size: 11px;font-weight: bold;"><em style="display: inline-block;height: .8em;width: .8em;background-color: {!!$v[1]!!};"></em> {!!$k!!} @if( round ($v[0] * 100 / $total) <= 6 ) [{!! round ($v[0] * 100 / $total) !!}%] @endif</div>&nbsp;&nbsp;
      </a>
			@endforeach
		</div>
		<div style="height: 21px;border-radius: 4px;">
			...
			<div class="device_item" style="
				...
				width:{!! $v[0] * 100 / $total !!}%;">
        <a href="?filter={!!$k!!}" style="display:inline-block;width:100%;height:100%;color: #fff;">
				@if( round ($v[0] * 100 / $total) > 6 ) {!! round ($v[0] * 100 / $total)!!}% @endif
        </a>
			</div>
			@endforeach
		</div>
  ...
  <br>
  <div id="rt_country"></div>
  <div style="clear: both;"></div>
</div>
<div style="width: 66.999%;display: inline-block;float: left;">
	<div id="rt_country_map" style="height: 450px;"></div>
</div>
