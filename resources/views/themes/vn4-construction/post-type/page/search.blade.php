        ...
        <div class="row">
            <div class="col-lg-8 mb-5 mb-lg-0">
                <div class="blog_left_sidebar">
                    ...
                    <?php 
                        $posts = get_posts('blog_post',['count'=>$post->number_of_post_in_each_page,'paginate'=>__t('page'), 'with'=>['category'],'callback'=>function($q) use ($search){
                            $q->where('title','LIKE','%'.$search.'%');
                        }]);
                     ?>

                     <h2 style="margin-bottom: 45px;">
                         @__('SEARCH RESULT FOR:') "{{ $search }}"
                     </h2>
                     @foreach($posts as $p)
