...
if( isset($image['link']) ){
	...
	if( $image['type_link'] === 'local' ){
		$image = cms_path().urldecode ($image['link']);
	}else{
		$image = $image['link'];
	}
...
if( $image ){
	...
	$value_input = $r->get('data');

	$info = pathinfo($image);

	$extension = get_image_mime_type($image);

	// if( $result['type_link'] == 'local' ){
	// 	$dir_upload = dirname($result['link']).'/';
	// }else{
		$dir_upload = 'uploads/thumbnails/';
	// }
	...
	foreach ($data as $k => $v) {
		if( isset($v['width']) || isset($v['height']) ){
			...
			if( $extension === 'image/jpg' ){
				...
				$filename = $dir_upload.$info['filename'].'-t'.time().'-'.$k.'-'.$width.'x'.$height.'.jpg';
				...
				$filename = $dir_upload.$info['filename'].'-t'.time().'-'.$k.'-'.$width.'x'.$height.'.png';
			...
			$max_width = isset($v['maxWidth'])?$v['maxWidth']:0;
			$max_height = isset($v['maxHeight'])?$v['maxHeight']:0;

			if( $extension === 'image/jpg' ){
				$img=imagecreatefromjpeg($info['dirname'].'/'.$info['basename']);
				$img = resizeJpeg($img,$max_width,$max_height);
				$filename = $dir_upload.$info['filename'].'-t'.time().'-'.$k.'-'.$max_width.'x'.$max_height.'.jpg';
				...
				$filename = $dir_upload.$info['filename'].'-t'.time().'-'.$k.'-'.$max_width.'x'.$max_height.'.png';
...
return ['message'=>apiMessage('Image Not Found!','error')];
