 ...
 do_action('before_get_data_'.$type);
  ...
  if( $admin_object2 ) $admin_object = $admin_object2;
   ...
   foreach($admin_object['fields'] as $k => $f){
      ...
      $checked = '';
      if( $groupby_column ){
        if( array_search($k,$groupby_column) !== false ){
          $checked = 'checked="checked"';
        }
      }
      if( isset($f['view']) && $f['view'] === 'relationship_onetomany' ){
...
<div class="x_panel vn4-bg-trans">
  <div class="x_content" style="box-shadow:none;">
      ...
      <ul class="pull-right advance-feature">
          ...
          <li class="dropdown">
              <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false"><i class="fa fa-filter" aria-hidden="true"></i> @__('Filters')</a>
