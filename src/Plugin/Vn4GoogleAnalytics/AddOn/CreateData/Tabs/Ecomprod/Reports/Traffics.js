    ...
    const tabs = [
        {
            ...
            data: () => {
                ...
                Object.keys(dataMockup.traffic.traffic_channel).forEach(function (key) {
                    data.push(
                        [
                            { v: new Date(dataMockup.traffic.traffic_channel[key].dayv), f: dataMockup.traffic.traffic_channel[key].dayf },
                            dataMockup.traffic.traffic_channel[key].channel['Organic Search'],
                            dataMockup.traffic.traffic_channel[key].channel['Direct'],
                            dataMockup.traffic.traffic_channel[key].channel['Paid Search'],
                            dataMockup.traffic.traffic_channel[key].channel['Referral'],
                            dataMockup.traffic.traffic_channel[key].channel['(Other)']
                        ]
                    );
                });
                return {
        ...
        },
        {
            ...
            data: () => {
                ...
                Object.keys(dataMockup.traffic.traffic_sourcemedium.key).forEach(function (key) {
                ...
                Object.keys(dataMockup.traffic.traffic_channel).forEach(function (key) {
                    ...
                    var data = [{ v: new Date(dataMockup.traffic.traffic_channel[key].dayv), f: dataMockup.traffic.traffic_channel[key].dayf }];

                    for (var i = 0; i < optionsLegend.length; i++) {
                        if (dataMockup.traffic.traffic_sourcemedium.data[key] && dataMockup.traffic.traffic_sourcemedium.data[key][optionsLegend[i]]) {
                            data.push(dataMockup.traffic.traffic_sourcemedium.data[key][optionsLegend[i]]);
                        } else {
                            data.push(null);
                        }
                    };
        ...
        },
        {
            ...
            data: () => {
                ...
                Object.keys(dataMockup.traffic.traffic_referrer.key).forEach(function (key) {
                    optionsLegend.push(key);
                    dataTotal[0].push(key);
                });
                ...
                Object.keys(dataMockup.traffic.traffic_channel).forEach(function (key) {
                    ...
                    for (var i = 0; i < optionsLegend.length; i++) {
                        if (dataMockup.traffic.traffic_referrer.data[key] && dataMockup.traffic.traffic_referrer.data[key][optionsLegend[i]]) {
                            data.push(dataMockup.traffic.traffic_referrer.data[key][optionsLegend[i]]);
                        } else {
                            data.push(0);
                        }
                    };
