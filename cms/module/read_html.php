...
if(!function_exists('get_img')){
    ...
    function get_shortcode($content){
    ...
    function get_content_html( $content ){
        ...
        $content = get_shortcode($content);

        $html = str_get_html($content);

        if(!$html) return '';
        
        $imgs = $html->find('img,a');

        $count = count($imgs);

        $mydomain = config('app.url');

        for ($i=0; $i < $count; $i++) { 

            if( $imgs[$i]->tag == 'img' ){

                // $img = new simple_html_dom('');

                // $img = $img->createTextNode($imgs[$i]);
                ...
                if( strpos($imgs[ $i ]->src, 'data:image') !== 0 ){
                    ...
                    if( is_url($src) && $pos === false ){
                        ...
                        $imgs[ $i ]->class = add_class( $imgs[ $i ]->class ,'img_external');

                    }else{

                        $imgs[ $i ]->class = add_class( $imgs[ $i ]->class ,'img_internal');

                        $src = strtok($src, '?');
                        $src = str_replace('../', '', $src);

                        // $src = asset(substr($src, strpos($src,'/uploads')));
                        $src = asset($src);
                        // $src = asset(substr($src, strpos($src,'../uploads') + 3 ));

                        $imgs[ $i ]->src = $src;

                    }

                    // $content = str_replace($imgs[ $i ], $img, $content);
                }

                
            }else{
                // $taga = new simple_html_dom('');
                // $taga = $taga->createTextNode($imgs[$i])->parent();
                ...
                if( is_url($href) && $pos === false ){
                    ...
                    $imgs[ $i ]->class = add_class( $imgs[ $i ]->class ,'link_external');

                    $imgs[ $i ]->rel = 'nofollow' ; 
                    
                }else{

                    $imgs[ $i ]->class = add_class( $imgs[ $i ]->class ,'link_internal');

                    $href = strtok($href, '?');

                    $href = str_replace('../', '', $href);

                    // $href = asset(substr($href, strpos($href,'/uploads')));
                    $href = asset($href);
                ...
                // $content = str_replace($imgs[ $i ], $taga, $content);
            }
