...
class LaravelLogViewer
{
    ...
    public static function setStorage($folder){
        self::$storage = $folder;
    }
    ...
    {
        $logsPath = storage_path(self::$storage);
     ...
     * @return array
     */
    public static function all( $file = null, &$levelResult = [] )
    {
            ...
            for ($i=0, $j = count($h); $i < $j; $i++) {
                foreach (self::$log_levels as $level) {
                    if (strpos(strtolower($h[$i]), '.' . $level) || strpos(strtolower($h[$i]), $level . ':')) {
                        ...
                        if( !isset($levelResult[$level]) ) $levelResult[$level] = 0;
                        $levelResult[$level] ++;
    ...
    {
        $files = glob(storage_path() . '/'.self::$storage.'/*.log');
