...
class Detail{
    ...
    public function __construct($idProduct, $timeType){
        $this->timeType = $timeType;
        $this->product = get_post('ecom_prod',$idProduct);
    ...
    */
    private function getOrders(){
        ...
        if( !isset($dataCache['orders']) ){
            $dataCache['orders'] = $this->product->related('ecom_order','ecom_prod',['count'=>1000000,'select'=>[
                '*', 
                ...
                $q->whereIn('order_status',array_keys(\Vn4Ecom\Order\SelectOptions::status()))
                    ...
                    ->whereIn('sale_channel',array_keys(\Vn4Ecom\Order\SelectOptions::saleChannel()));
            }]);
        }
    ...
    */
    private function getReviews(){
        ...
        if( !isset($dataCache['reviews']) ){
            ...
            $dataCache['reviews'] = get_posts('ecom_prod_review',['count'=>1000000,'select'=>['id','rating'], 'callback'=>function($q) use ($productID) {
                $q->where('review_status', 'approved')
                ->whereDate('created_at', '>=', $this->startDate)
                ->whereDate('created_at','<=', $this->endDate)
                ->where('ecom_prod', $productID);
            }]);
        ...
        foreach( $rangeTime as $date ){
            ...
            if( isset( $orderGroups[$date] ) ){
                ...
                $orderGroupByDate[] = [
                    ...
                    // array_reduce($orderGroups[$date]->toArray(), function($carry, $order){
    ...
    */
    public function getOrderReport(){
        ...
        foreach( $orders as $order ){
            ...
            // $order->discount = \Vn4Ecom\Cart\Model\Cart::getDiscount( $order->discount, $order->pricing['revenue'] );
